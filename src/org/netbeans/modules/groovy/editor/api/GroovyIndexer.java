...
System.out.println(" --- JJ iiiiiiiiiii GroovyIndexer.index() cache folder=" + context.getIndexFolder().getPath());
...
System.out.println("1) --- iiiiiiiiiii GroovyIndexer.index() START time="+System.currentTimeMillis());
        GroovyParserResult r = (GroovyParserResult) AstUtilities.getParseResult(parserResult);
        ASTNode root = AstUtilities.getRoot(r);

        if (root == null) {
            System.out.println("2) --- iiiiiiiiiii GroovyIndexer.index() root==NULL  time="+System.currentTimeMillis());
            return;
        }
...
System.out.println("3) --- iiiiiiiiiii GroovyIndexer.index() DOC" );  
...
System.out.println("4) --- jjjjjjjjjjjjjjjjjjjjj GroovyIndexer.index() DOC filCount=" + 
            filesIndexed + 
            "; time (ms)=" + (System.currentTimeMillis()-indexerThisStartTime)
            + "; fileName=" + r.getSnapshot().getSource().getFileObject() );
...
System.out.println("5) --- iiiiiiiiiii GroovyIndexer.index() TIME(ms)=" + (System.currentTimeMillis()-indexerThisStartTime));                            
    }
...
System.out.println("123 MARK GIRTY " + i.getRelativePath());                    
