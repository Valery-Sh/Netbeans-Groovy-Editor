...
public class ResolveVisitorDelegate extends ClassCodeExpressionTransformer {

    protected TypeCache typeCache;
    private CompilationUnit compilationUnit;
    private SourceUnit source;
    private EditorResolveVisitor owner;
    ...
    public ResolveVisitorDelegate(org.codehaus.groovy.control.CompilationUnit cu) {
        this(cu, false);
    }

    public ResolveVisitorDelegate(CompilationUnit cu, EditorResolveVisitor owner) {
        this(cu, false);
        this.owner = owner;

    }

    public ResolveVisitorDelegate(CompilationUnit cu, boolean b) {
...
System.out.println("????????????????? ************* startResolving startResolving startResolving ");        
        this.source = source;
...
/////////////////////////////
    ...
    protected void visitConstructorOrMethod(MethodNode node, boolean isConstructor) {
        ...
        for (Parameter p : paras) {
            String nm = p.getType().getName();

            p.setInitialExpression(transform(p.getInitialExpression()));
            resolveOrFail(p.getType(), p.getType());
/*if ( "saySay".equals(node.getName())) {
    System.out.println("METHOD " + node.getName() + ": PARAMETER name=" + p.getName() + "; type = " + p.getType() + "; type.name=" +  p.getType().getName());
    
//???                ClassNode paramClassNode = createClassNode(varElem, bounds, genericsTypes);
    System.out.println("=======================================================");
    PrintUtils.printClassNode(p.getType());
}           
*/ 
            visitAnnotations(p);
        }
...
/*        if ("mySay2".equals(node.getName())) {
            GenericsType[] gtypes = node.getGenericsTypes();
            if (gtypes != null) {
                System.out.println(" MYSAY2  GenericsType[].length=" + gtypes.length);
                for (int i = 0; i < gtypes.length; i++) {
                    System.out.println(" MYSAY2  GenericsType[" + i + "] = " + gtypes[i]);
                }
            }
            Parameter[] pars = node.getParameters();
            System.out.println(" MYSAY2  parameters.length=" + pars.length);
            for (int i = 0; i < pars.length; i++) {
                System.out.println(" MYSAY2 --------- parameter index=" + i + "; paramm name=" + pars[i].getName() + "; param type=" + pars[i].getType());
                GenericsType[] ptypes = pars[i].getType().getGenericsTypes();
                if (ptypes != null) {
                    System.out.println(" MYSAY2 param[" + i + "].GenericsType[].length=" + ptypes.length);
                    for (int n = 0; n < ptypes.length; n++) {
                        System.out.println(" MYSAY2 param[" + i + "].GenericsType[" + n + "] = " + ptypes[n]);
                    }
                }
            }

            System.out.println(" MYSAY2  END _______________________________________");
...
*/
    ...
    protected void addError(String msg, ASTNode expr) {
        ...
        compilationUnit.getErrorCollector().addError(
                new SyntaxErrorMessage(new SyntaxException(msg + '\n', line, col), source));

    }

    private void resolveOrFail(ClassNode type, String msg, ASTNode node) {
//System.out.println("START resolveOrFail(ClassNode type, String msg, ASTNode node)");                
        
        if (resolve(type)) {
//System.out.println("1 resolveOrFail(ClassNode type, String msg, ASTNode node)");                
            
            return;
        }
//System.out.println("2 resolveOrFail(ClassNode type, String msg, ASTNode node)");                
        
        if (resolveToInner(type)) {
//System.out.println("3 resolveOrFail(ClassNode type, String msg, ASTNode node)");                
            
            return;
        }
//System.out.println("4 addError resolveOrFail(ClassNode type, String msg, ASTNode node)");                
        
//        System.out.println("UNABLE RESOLVE name=" + type.getName());
...
//new ExceptionMessage(cfe, true, source
...
//System.out.println("!!!! 0000000001 !!!! ~~~~~ resolve " + type.getName());                
        resolveGenericsTypes(type.getGenericsTypes());
...
//System.out.println("!!! 0000003  !!!! ~~~~~ resolve gt=" + gt );
...
//System.out.println("!!! 0000004  !!!! ~~~~~ resolve" );
...
//System.out.println("!!! 0000004  !!!! ~~~~~ resolve type=");
...
//System.out.println("#### resolveFromDefaultImports");                
    ...
    private boolean resolveFromCompileUnit(ClassNode type) {
        ...
        ClassNode cuClass = compileUnit.getClass(jname);
        ...
        /*
         * ClassNode cuClass = compileUnit.getClass(type.getName()); if (cuClass
         * != null) { if (type != cuClass) type.setRedirect(cuClass); return
         * true; } return false;
         */
    }
...
//System.out.println("#### resolveAliasFromModule");        
...
//System.out.println("#### resolveFromModule type name=" + type.getName());                
...
//System.out.println("#### 1 resolveFromModule RESOLVED type name=" + type.getName());                                
                return true;
            }
        }
        if (newNameUsed) {
...
//System.out.println("#### 2 resolveFromModule RESOLVED type name=" + type.getName());                                                
                return true;
            }
            ...
            if (module.hasPackageName()) {
                ...
                if (resolve(tmp, false, false, false)) {
                    ...
                    type.setRedirect(tmp.redirect());
//System.out.println("#### 3 resolveFromModule RESOLVED type name=" + type.getName());                                                    
                    return true;
                }
            }
            ...
            for (ImportNode importNode : module.getStaticImports().values()) {
                if (importNode.getFieldName().equals(name)) {
                    ...
                    if (resolve(tmp, false, false, true)) {
                        if ((tmp.getModifiers() & Opcodes.ACC_STATIC) != 0) {
                            type.setRedirect(tmp.redirect());
//System.out.println("#### 4 resolveFromModule RESOLVED type name=" + type.getName());                                                            
                            return true;
                        }
                    }
                }
            ...
            for (ImportNode importNode : module.getStarImports()) {
                ...
                if (resolve(tmp, false, false, true)) {
                    ambiguousClass(type, tmp, name);
                    type.setRedirect(tmp.redirect());
//System.out.println("#### 5 resolveFromModule RESOLVED type name=" + type.getName());                                                    
                    return true;
                }
            }
            ...
            for (ImportNode importNode : module.getStaticStarImports().values()) {
                ...
                if (resolve(tmp, false, false, true)) {
                    if ((tmp.getModifiers() & Opcodes.ACC_STATIC) != 0) {
                        ambiguousClass(type, tmp, name);
                        type.setRedirect(tmp.redirect());
//System.out.println("#### 6 resolveFromModule RESOLVED type name=" + type.getName());                                                        
                        return true;
                    }
                }
...
//System.out.println("#### 7 resolveFromModule UNRESOLVED type name=" + type.getName());                                        
        return false;
    }
    ...
    public boolean resolveToClass(ClassNode type) {
        ...
        // scheduled then
/*
         * if ("java.util.ArrayList".equals(name)) { System.out.println("1)
         * RESOLVE TO CLASS ARRAYLIST CLASS (((((((((((((((((((( " + name); }
         */
        ...
        try {
            ...
            cachedClasses.put(name, NO_CLASS);
//            System.out.println("1) CLASS NOT FOUND name=" + name + "; msg=" + cnfe.getMessage());
            boolean b = resolveToScript(type);
//            System.out.println("2) CLASS NOT FOUND name=" + name);
            return b;
        } catch (CompilationFailedException cfe) {
//            System.out.println("3) CLASS NOT FOUND name=" + name + "; msg=" + cfe.getMessage());

            compilationUnit.getErrorCollector().addErrorAndContinue(new ExceptionMessage(cfe, true, source));
//            System.out.println("4) CLASS NOT FOUND name=" + name);
            boolean b = resolveToScript(type);
//            System.out.println("2) CLASS NOT FOUND name=" + name);
            return b;
         ...
         * return false; }
         */
//        System.out.println("3) RESOLVE TO CLASS  CLASS (((((((((((((((((((( " + name);

        if (cls == null) {
            return false;
        }
//        System.out.println("4) RESOLVE TO CLASS  CLASS (((((((((((((((((((( " + name);

        ClassNode cn = ClassHelper.make(cls);
        cachedClasses.put(name, cn);
        type.setRedirect(cn);
//        System.out.println("5) RESOLVE TO CLASS  CLASS (((((((((((((((((((( " + name + "; isResolved=" + type.isResolved());
...
//System.out.println("#### 11111 ~~~~~ getClass name=" + exp.getType().getName());                                    
...
//System.out.println("00001  ~~~~~ transformVariableExpression=" + ve);
        if (!(v instanceof DynamicVariable) && !checkingVariableTypeInDeclaration) {
//System.out.println("00002  ~~~~~ transformVariableExpression=" + ve);
...
//System.out.println("00003  ~~~~~ transformVariableExpression ve.getName()=" + ve.getName());
...
//System.out.println("00004  ~~~~~ transformVariableExpression t.getName()=" + t.getName());
                
                isClass = resolve(t);
//System.out.println("00005  ~~~~~ transformVariableExpression t.getName()=" + t.getName());
...
//System.out.println("00006  ~~~~~ transformVariableExpression t.getName()=" + t.getName());
...
//System.out.println("00007  ~~~~~ transformVariableExpression ve.type=" + ve.getType() + "ve.type.name=" + ve.getType().getName() + "; isUsingGenerics=" + ve.getType().isUsingGenerics() );
        
        resolveOrFail(ve.getType(), ve);
//System.out.println("00008  ~~~~~ transformVariableExpression ve=" + ve.getType() + "ve.type.name=" + ve.getType().getName());
...
//System.out.println("#### 000 ~~~~~ transformBinaryExpression be=" + be);                            
        Expression left = transform(be.getLeftExpression());
//System.out.println("#### 001 ~~~~~ transformBinaryExpression left=" + left);                                    
...
//System.out.println("#### 111 ~~~~~ transformBinaryExpression=" + left);                    
...
//System.out.println("#### 22222 ~~~~~ transformDeclarationExpression=" + oldLeft);
        checkingVariableTypeInDeclaration = true;
        Expression left = transform(oldLeft);
//System.out.println("#### 33333 ~~~~~ transformDeclarationExpression=" + left);        
...
//System.out.println("#### 44444 ~~~~~ transformDeclarationExpression=" + de);                    
            fixDeclaringClass(de);
//System.out.println("#### 55555 ~~~~~ transformDeclarationExpression=" + de);                    
...
//System.out.println("#### 77777 ~~~~~ transformDeclarationExpression=" + newDeclExpr);        
...
//            System.out.println("1) ===== ResolveVisitorNew VISIT ANNOTATION name = " + annType.getName());
            if (annType.isResolved()) {
                Class annTypeClass = annType.getTypeClass();
//                System.out.println("2) ===== ResolveVisitorNew VISIT ANNOTATION name = " + annType.getName() + "; typeClass=" + annTypeClass);
    ...
    public void visitClass(ClassNode node) {
        ...
        genericParameterNames = new HashMap<String, GenericsType>(genericParameterNames);
        currentClass = node;
//System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! VISIT CLASS BEFORE resolveGenericsHeader classNode=" + node);
        resolveGenericsHeader(node.getGenericsTypes());
//        PrintUtils.printClassNode(node);
//System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! VISIT CLASS AFTER resolveGenericsHeader classNode=" + node);
...
//System.out.println("!!! VISIT CLASS BEFORE visit SUPER classNode=" + node);
        super.visitClass(node);
//System.out.println("!!! VISIT CLASS AFTER visit SUPER classNode=" + node);
...
void printGenericsTypes(GenericsType[] types,String str) {
System.out.println(str + " ================= PRINT GenericsTypes[] resolveGenericsHeader(GenericsType[] ===================================" );
        for (GenericsType type : types) {
            System.out.println("1. gt = " + type);
            System.out.println("2. gt.getType() = " + type.getType());
            System.out.println("3. gt.getType().getName() = " + type.getType().getName());
            ClassNode[] bounds = type.getUpperBounds();
            if ( bounds == null || bounds.length == 0 ) {
            System.out.println("4. gt.getUpperBounds = NULL");
                
            } else {
                System.out.println(" -------------- PRINT Upper Bounds --------------- length=" + bounds.length );
                for ( ClassNode bound : bounds) {
                    System.out.println(" ---- 1) bound.type " + bound);
                    System.out.println(" ---- 2) bound.type.name " + bound.getName());
                    System.out.println(" ---- 3) isUsingGenerics = " + bound.isUsingGenerics());
                    System.out.println(" ---- 4) isGenericsPlaceholder = " + bound.isGenericsPlaceHolder());                    
                    if ( bound.getGenericsTypes() != null && bound.getGenericsTypes().length > 0 ) {
                        printGenericsTypes(bound.getGenericsTypes(), "BBBBBBB");
                    }
...
System.out.println("================= END PRINT GenericsTypes[] resolveGenericsHeader(GenericsType[] ===================================" );
        
}        
private void resolveGenericsHeader(GenericsType[] types) {
//if ( types == null ) {
//    System.out.println("START resolveGenericsHeader(GenericsType[] types) types = NULL !!!!");    
//}       
        if (types == null) {
            return;
        }
//System.out.println("START resolveGenericsHeader(GenericsType[] types) length=" + types.length);
...
//System.out.println("~~~~~~~~~~~~~~~~~~~............. BEFORE RESOLVE OR FAIL ~~~~~ upperBound.type="+upperBound+ "; genericsType.name=" + type.getName() + "; genericsType.type=" + type.getType());                    
                    
                    resolveOrFail(upperBound, classNode);
//System.out.println("~~~~~~~~~~~~~~~~~~~............. AFTER RESOLVE OR FAIL ~~~~~ upperBound.type="+upperBound+ "; genericsType.name=" + type.getName() + "; genericsType.type=" + type.getType());                    
