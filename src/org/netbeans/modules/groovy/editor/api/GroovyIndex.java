...
import org.netbeans.modules.groovy.editor.api.elements.*;
...
//Language lan = LanguageRegistry.getInstance().getLanguageByMimeType(clusterUrl);        
//lan.
    }
    ...
    public static GroovyIndex get(FileObject... roots) {
        try {
            return new GroovyIndex(QuerySupport.forRoots(
                    GroovyIndexer.Factory.NAME,
                    GroovyIndexer.Factory.VERSION,
    ...
    public Set<IndexedClassElement> getClassElements(String name, final QuerySupport.Kind kind, boolean includeAll,
        boolean skipClasses, boolean skipModules) {
        return getClassElements(name, kind, includeAll, skipClasses, skipModules, null);
    }
...
System.out.println("^^^^^^^^^^^^^^^^^ GroovyIndexer name=" + name);
        if (name != null) {
            if (name.endsWith(".")) {
...
System.out.println("^^^^^^^^^^^^^^^^^ GroovyIndexer name=" + name);
        ...
        final Set<IndexedClassElement> classes = new HashSet<IndexedClassElement>();
...
//                            }
            ...
            classes.add(createClassElement(fqn, simpleName, map));
        }
...
//        }
    ...
    private IndexedClassElement createClassElement(String fqn, String simpleName, IndexResult map) {
        ...
        IndexedClassElement c =
            IndexedClassElement.create(this, simpleName, fqn, map, attrs, flags);
