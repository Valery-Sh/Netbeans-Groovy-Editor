...
import static org.netbeans.modules.groovy.editor.stc.StaticTypeCheckingSupportMy.*;
 ...
 */
public class StaticTypeCheckingVisitorMy extends ClassCodeVisitorSupport {
    ...
    public StaticTypeCheckingVisitorMy(SourceUnit source, ClassNode cn) {
        this.source = source;
        this.classNode = cn;
...
System.out.println("***** 1 StaticTypeCheckingVisitorMy receiver.name = " + currentReceiver.getName() + "; method=" + name);                                    
for (ClassNode a : args) {
System.out.println("***** 2 StaticTypeCheckingVisitorMy arg.name = " + a.getName() + "; mn.isEmpty()=" + mn.isEmpty());                                    
}
                
                if (!mn.isEmpty()) {
System.out.println("***** 3 StaticTypeCheckingVisitorMy BEFORE typeCheckMethodsWithGenerics");                    
                    typeCheckMethodsWithGenerics(currentReceiver, args, mn, call);
                    chosenReceiver = currentReceiver;
                    break;
                }
            }
System.out.println("***** 4 StaticTypeCheckingVisitorMy mn.isEmpty()=" + mn.isEmpty());                                
            if (mn.isEmpty()) {
                addStaticTypeError("My1 Cannot find matching method " + receiver.getName() + "#" + toMethodParametersString(name, args), call);
            } else {
                if (isCallOnClosure) {
...
System.out.println("***** StaticTypeCheckingVisitorMy receiver.name = " + receiver.getName() + "; method=" + name);                    
        if (methods.isEmpty()) {

            addStaticTypeError("My Cannot find matching method " + receiver.getName() + "#" + toMethodParametersString(name, args), expr);
    ...
    private List<MethodNode> chooseBestBethod(final ClassNode receiver, Collection<MethodNode> methods, ClassNode... args) {
        ...
        for (MethodNode m : methods) {
            ...
            /*
                ...
                Person p = foo(b)
             */
        if ( "myGen".equals(m.getName())) {
System.out.println(" 1 &&&&&&&&&&&&&&&& visitConstructorOrMethod");            
                ClassNode cn = m.getParameters()[0].getType();
                
                System.out.println("5 ******************** T1_1_2 PARAMETER INFO TYPE   param obj myMethod.getParameters()[0]=" + m.getParameters()[0]);
                
                System.out.println("5.0 -------------------------- TYPE =  T1_1_2 p.getType().getGenericsTypes()=" + cn.getGenericsTypes());
                System.out.println("5.1 -------------------------- TYPE =  T1_1_2 p.getType().isGenericsPlaceHolder()=" + cn.isGenericsPlaceHolder());                
                System.out.println("5.2 -------------------------- TYPE =  T1_1_2 p.getType().isRedirectNode()=" + cn.isRedirectNode());                
                System.out.println("5.3 -------------------------- TYPE =  T1_1_2 p.getType().isUsingGenerics()=" + cn.isUsingGenerics());                
                System.out.println("5.4 -------------------------- TYPE =  T1_1_2 p.getType().getName()=" + cn.getName());                
                System.out.println("5.5 -------------------------- TYPE =  T1_1_2 p.getType() object = " + cn);                


        }

            Parameter[] params = parameterizeArguments(receiver, m);
System.out.println("***** 1 StaticTypeCheckingVisitorMy.chooseBestBethod receiver.name = " + receiver.getName() + "; method=" + m.getName());                                    
for ( Parameter p : params) {
System.out.println("***** 2 StaticTypeCheckingVisitorMy.chooseBestBethod param.name = " + p.getName() + "; param.type=" + p.getType());                                        
}            
        if ( "myGen".equals(m.getName())) {
System.out.println(" &&&&&&&&&&&&&&&& AFTER PARAMETRIZIZE");            
                ClassNode cn = params[0].getType();
                
                System.out.println("6 ******************** T1_1_2 PARAMETER INFO TYPE   param obj myMethod.getParameters()[0]=" + params[0]);
                
                System.out.println("6.0 -------------------------- TYPE =  T1_1_2 p.getType().getGenericsTypes()=" + cn.getGenericsTypes());
                System.out.println("6.1 -------------------------- TYPE =  T1_1_2 p.getType().isGenericsPlaceHolder()=" + cn.isGenericsPlaceHolder());                
                System.out.println("6.2 -------------------------- TYPE =  T1_1_2 p.getType().isRedirectNode()=" + cn.isRedirectNode());                
                System.out.println("6.3 -------------------------- TYPE =  T1_1_2 p.getType().isUsingGenerics()=" + cn.isUsingGenerics());                
                System.out.println("6.4 -------------------------- TYPE =  T1_1_2 p.getType().getName()=" + cn.getName());                
                System.out.println("6.5 -------------------------- TYPE =  T1_1_2 p.getType() object = " + cn);                
...
System.out.println("***** 3 StaticTypeCheckingVisitorMy.chooseBestBethod allPMatch = " + allPMatch + "; lastArgMatch=" + lastArgMatch + "; dist =" + dist) ;                                                        
                if (dist>=0 && dist<bestDist) {
                    bestChoices.clear();
                    bestChoices.add(m);
                    bestDist = dist;
                } else if (dist>=0 && dist==bestDist) {
System.out.println("***** 4 StaticTypeCheckingVisitorMy.chooseBestBethod allPMatch = " + allPMatch + "; lastArgMatch=" + lastArgMatch + "; dist =" + dist + "; bestDist=" + bestDist) ;                                                        
...
System.out.println("1) A CUSTOM MYSAY2  receiver=" + receiver);
if ( receiver.getGenericsTypes() != null ) {
System.out.println("1.0) A CUSTOM MYSAY2  receiver=" + receiver + "; receiver.getGenericsTypes.length=" + receiver.getGenericsTypes().length);
        ...
        if (redirectReceiverTypes==null) {
            ...
            // at the method level
System.out.println("2) A CUSTOM MYSAY2  redirectReceiverTypes == NULL");
...
System.out.println("2) A CUSTOM MYSAY2  receiverParameterizedTypes == NULL");
            
            receiverParameterizedTypes = redirectReceiverTypes;
        }
        /////////// START MY ////////////////
        MethodNode mn = m;
System.out.println("2) A CUSTOM MYSAY2  receiver=" + receiver);
        
        if ("mySay2".equals(mn.getName())) {
System.out.println("A CUSTOM MYSAY2  receiver=" + receiver);
            
            //GenericsType[] gtypes = mn.getGenericsTypes();
            GenericsType[] gtypes = receiverParameterizedTypes;
            if (gtypes != null) {
                System.out.println("A CUSTOM MYSAY2  GenericsType[].length=" + gtypes.length);
                for (int i = 0; i < gtypes.length; i++) {
                    System.out.println("A CUSTOM MYSAY2  GenericsType[" + i + "] = " + gtypes[i]);
                }
            }
            Parameter[] pars = mn.getParameters();
            System.out.println("A CUSTOM MYSAY2  parameters.length=" + pars.length);
            for (int i = 0; i < pars.length; i++) {
                System.out.println("A CUSTOM MYSAY2 --------- parameter index=" + i + "; param name=" + pars[i].getName() + "; param type=" + pars[i].getType());
                GenericsType[] ptypes = pars[i].getType().getGenericsTypes();
                if (ptypes != null) {
                    System.out.println("A CUSTOM MYSAY2 param[" + i + "].GenericsType[].length=" + ptypes.length);
                    for (int n = 0; n < ptypes.length; n++) {
                        System.out.println("A CUSTOM MYSAY2 param[" + i + "].GenericsType[" + n + "] = " + ptypes[n]);
                    }
                }
            }

            System.out.println("_____________ A CUSTOM MYSAY2  END _______________________________________");
...
System.out.println("7) IN CYCLE CUSTOM MYSAY2  genericsTypes.length <> 1=");
                    params[i] = methodParameter;
                    
                }
            } else {
System.out.println("8) IN CYCLE CUSTOM MYSAY2  paramType.isUsingGenerics()=" + paramType.isUsingGenerics());
                params[i] = methodParameter;
            }
        }
        
        if ("mySay2".equals(mn.getName())) {
System.out.println("B CUSTOM MYSAY2  receiver=" + receiver);
            
            //GenericsType[] gtypes = mn.getGenericsTypes();
            GenericsType[] gtypes = receiverParameterizedTypes;
            if (gtypes != null) {
                System.out.println("B CUSTOM MYSAY2  GenericsType[].length=" + gtypes.length);
                for (int i = 0; i < gtypes.length; i++) {
                    System.out.println("B CUSTOM MYSAY2  GenericsType[" + i + "] = " + gtypes[i]);
                }
            }
            Parameter[] pars = params;
            System.out.println("B CUSTOM MYSAY2  parameters.length=" + pars.length);
            for (int i = 0; i < pars.length; i++) {
                System.out.println("B CUSTOM MYSAY2 --------- parameter index=" + i + "; param name=" + pars[i].getName() + "; param type=" + pars[i].getType());
                GenericsType[] ptypes = pars[i].getType().getGenericsTypes();
                if (ptypes != null) {
                    System.out.println("B CUSTOM MYSAY2 param[" + i + "].GenericsType[].length=" + ptypes.length);
                    for (int n = 0; n < ptypes.length; n++) {
                        System.out.println("B CUSTOM MYSAY2 param[" + i + "].GenericsType[" + n + "] = " + ptypes[n]);
                    }
                }
            }

            System.out.println("B CUSTOM MYSAY2  END _______________________________________");
