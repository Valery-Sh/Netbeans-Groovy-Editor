...
public final class GroovyMetaElementHandler {

    private static final Logger LOG = Logger.getLogger(MetaElementHandler.class.getName());

    private final ParserResult info;

    private GroovyMetaElementHandler(ParserResult info) {
        this.info = info;
        
    }

    public static GroovyMetaElementHandler forCompilationInfo(ParserResult info) {
        return new GroovyMetaElementHandler(info);
    }

    
    public Map<MethodSignature, ? extends CompletionItem> getMethods(ClassNode node,
            String prefix, int anchor, boolean nameOnly) {
        
        if ( ! node.isResolved() ) {
            return Collections.emptyMap();            
        }
        Class clz = node.getTypeClass();
    ...
    public Map<FieldSignature, ? extends CompletionItem> getFields(ClassNode node,
